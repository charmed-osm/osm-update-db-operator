...
def _update_nsr(osm_db):
    ...
    for nsr in nsrs.find():
        ...
        _remove_namespace_from_k8s(nsrs, nsr)
...
def _update_k8scluster(osm_db):
    ...
    for k8scluster in k8sclusters.find():
        if k8scluster["_admin"].get("helm-chart") and k8scluster["_admin"]["helm-chart"].get("id"):
            if k8scluster["_admin"]["helm-chart"]["id"].startswith(namespace):
                k8scluster["_admin"]["helm-chart"]["id"] = k8scluster["_admin"]["helm-chart"][
                    "id"
                ].replace(namespace, "", 1)
        if k8scluster["_admin"].get("helm-chart-v3") and k8scluster["_admin"]["helm-chart-v3"].get(
            "id"
        ):
...
def _upgrade_mongo_10_12(mongo_uri):
    ...
    logger.info("Entering in upgrade function")
    myclient = MongoClient(mongo_uri)
    osm_db = myclient["osm"]
    _update_nsr(osm_db)
    _update_vnfr(osm_db)
    _update_k8scluster(osm_db)
        ...
        for nslcmop in nslcmops.find():
            if nslcmop.get("operationParams"):
                if nslcmop["operationParams"].get("additionalParamsForVnf") and isinstance(
                    ...
                    string_param = json.dumps(nslcmop["operationParams"]["additionalParamsForVnf"])
...
def _patch_1837(mongo_uri):
    ...
    myclient = MongoClient(mongo_uri)
    osm_db = myclient["osm"]
    _update_nslcmops_params(osm_db)
    _update_vnfrs_params(osm_db)
